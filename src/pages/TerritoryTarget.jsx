...
import { useGetTerritoryTargetSummaryQuery } from "../redux/features/teritory/territoryTargetApi";
...
const TerritoryTarget = () => {
  ...
  const { data, isLoading, isError } = useGetTerritoryTargetSummaryQuery();
  ...
  const currentTerritories = filteredTerritories.slice(
    startIndex,
    startIndex + territoriesPerPage
  );

  // ✅ Redirect to Set Target page
  const handleSetTargetClick = (territory) => {
    navigate(`/territory/${territory._id}/set-target`, { state: { territory } });
  };
            ...
            <table className="table w-full border-collapse">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="text-right">Total Target Amount</th>
                  <th className="text-right">Set Target</th>
                </tr>
              </thead>
              <tbody>
                {currentTerritories.length > 0 ? (
                  currentTerritories.map((territory, idx) => (
                    <tr key={territory._id} className="border-b">
                      <td className="text-center">{startIndex + idx + 1}</td>
                      <td>{territory.territoryName}</td>
                      <td>{territory.areaManager?.name || "-"}</td>
                      <td>{territory.zonalManager?.name || "-"}</td>
                      <td className="text-center">{territory.totalProduct}</td>
                      <td className="text-right">{territory.totalTargetQty}</td>
                      <td className="text-right">{territory.totalTargetAmount}</td>
                      <td className="text-right">
                        <button
                          className="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600"
                          onClick={() => handleSetTargetClick(territory)}
                        >
                          Set Target
                        </button>
                      </td>
...
export default TerritoryTarget;
