...
//     { key: 'quantityIn', label: 'Quantity', sortable: true, render: (value) => Number(value || 0).toLocaleString() },
...
//                 <span>DEPOT</span>
//                 <ChevronRight size={14} className="text-gray-400" />
//                 <span className="text-gray-900 font-bold">STOCK IN</span>
//               </h2>
//             </div>
//           </div>
...
//       {/* Data Table */}
//       <Card title="Stock In Records" subtitle={`Showing ${currentItems.length} of ${stockList.length} records`}>
//         <Table
//           columns={columns}
//           data={currentItems}
//           sortable
...
//               ><BsArrowLeftSquareFill className="inline mr-1" />
//                 Previous
//               </Button>
...
//               >
//                 Next
//                 <BsArrowRightSquareFill className="inline ml-1" />
//               </Button>
//             </div>
//           </div>
//         </Card>
...
const DepotStockIn = () => {
  ...
  // Table columns configuration (fixed NaN & Invalid Date)
  const columns = [
    { key: 'slNo', label: 'Sl. No.', sortable: false, render: (value, row, index) => startIndex + index + 1 },
    { key: 'productName', label: 'Product Name', sortable: true },
    { key: 'brand', label: 'Brand', sortable: true },
    ...
    { key: 'packSize', label: 'Pack Size', sortable: true },
    { key: 'category', label: 'Category', sortable: true },
    { key: 'actualPrice', label: 'Actual Price', sortable: true, render: (value) => `৳${isNaN(Number(value)) ? 0 : Number(value).toLocaleString()}` },
    { key: 'batch', label: 'Batch', sortable: true },
    { key: 'quantityIn', label: 'Quantity', sortable: true, render: (value) => isNaN(Number(value)) ? 0 : Number(value).toLocaleString() },
    { key: 'stockAvailableBefore', label: 'Stock Before', sortable: true, render: (value) => isNaN(Number(value)) ? 0 : Number(value).toLocaleString() },
    { key: 'stockAvailableAfter', label: 'Stock After', sortable: true, render: (value) => isNaN(Number(value)) ? 0 : Number(value).toLocaleString() },
    { key: 'expireDate', label: 'Expire Date', sortable: true, render: (value) => {
      const date = new Date(value);
      return isNaN(date.getTime()) ? "-" : date.toLocaleDateString();
    } },
    { key: 'stockInDate', label: 'Stock In Date', sortable: true, render: (value) => {
      const date = new Date(value);
      return isNaN(date.getTime()) ? "-" : date.toLocaleDateString();
    } },
  ];
