...
import { useGetAllRidersQuery } from "../../../redux/features/rider/riderApi";
...
const ApprovePendingInstituteOrders = () => {
  const { data, isLoading, refetch } = useGetPendingInstituteOrdersQuery(undefined, {
  ...
  const [assignRiderApi] = useAssignRiderToInstituteOrderMutation();
  ...
  if (!orders.length) {
    return (
      <div className="text-center mt-12">
        <p className="text-gray-500 mb-4 mt-60">No pending institute orders</p>
        <button
  ...
  const handlePrintInvoice = (order) => {
    navigate("/institutes/invoice-print", { state: { orderId: order._id } });
  };
  ...
  const handleOpenOrderModal = async (orderId) => {
    try {
      ...
      if (!latestOrder) {
        toast.error("Order is no longer available or already updated.");
        return;
      }
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {orders.map((order, index) => {
              const institute = order.customerId;
              ...
              return (
                <React.Fragment key={order._id}>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      Rs. {order.totalPayable?.toFixed(2) || "0.00"}
                    </td>
...
export default ApprovePendingInstituteOrders;
