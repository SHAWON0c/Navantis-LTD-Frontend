...
/* ---------------------- OrderTable ---------------------- */
const OrderTable = ({ statusFilter }) => {
  const { data, isLoading } = useGetAllInstituteOrdersQuery(
  ...
  const columns = [
    { title: "SL No", key: "serial", render: (_, __, index) => index + 1, width: 60 },
    {
      ...
      dataIndex: ["customerId", "instituteName"],
      key: "institute",
      render: (text) => text || "-",
    },
    {
    ...
    },
    {
      ...
      key: "total",
      render: (text) => `Rs. ${text?.toFixed(2) || "0.00"}`,
    },
    {
...
/* ---------------------- SidebarActions ---------------------- */
const SidebarActions = ({ statusFilter, setStatusFilter }) => {
  const buttons = ["Confirmed", "Assigned", "Delivered", "Paid", "Outstanding"];

  return (
    <div className="w-60 bg-white rounded-xl shadow-lg p-6 flex flex-col gap-2 h-fit sticky top-6">
      {buttons.map((btn) => (
        <Button
          key={btn}
          type={statusFilter === btn ? "primary" : "default"}
          size="large"
          className={`font-semibold ${statusFilter === btn ? "bg-blue-600 text-white" : ""}`}
...
/* ---------------------- Main Component ---------------------- */
const InstituteOrderInvoiceAndPayment = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50 p-6">
            ...
            <div className="bg-white text-gray-500 flex items-center px-3 sm:px-4 md:px-6 py-2 sm:h-12">
              <h2 className="flex flex-wrap items-center text-xs sm:text-sm md:text-base font-semibold text-gray-800 gap-1 sm:gap-2">
                ...
                <span className="text-gray-900 font-bold">
                  INVOICE & PAYMENT
                  <span className="bg-blue-100 text-sm text-blue-600 p-1 ml-2">
      ...
      <div className="flex gap-6">
        ...
        <div className="w-60">
          <SidebarActions statusFilter={statusFilter} setStatusFilter={setStatusFilter} />
        </div>
      </div>
    </div>
  );
...
export default InstituteOrderInvoiceAndPayment;
