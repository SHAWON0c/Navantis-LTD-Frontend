const PrintWarehouseStockList = ({ products, summary }) => {
  ...
  const filteredProducts = products.map((p) => ({
    ...
    batch: p.batch || "N/A",
    expire: p.expireDate ? new Date(p.expireDate).toLocaleDateString("en-GB").replace(/\//g, "-") : "N/A",
  ...
  const handlePrint = () => {
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        ...
        ">
          <span style="
            ...
            width: 8px; height: 8px; background: #2E86C1; border-radius: 50%;
          "></span>
    ...
    const filteredTableContent = `
      <div style="margin-bottom: 20px; padding: 10px; border: 1px solid #B2BEB5; border-radius: 4px; background: #f9f9f9;">
        <h3 style="font-size: 12px; font-weight: bold; text-align: center;">Warehouse Stock Summary</h3>
    ...
    newWindow.document.write(`
      <html>
        <head>
          <title>Warehouse Stock List - ${today}</title>
          ${styles}
          <style>
...
export default PrintWarehouseStockList;
